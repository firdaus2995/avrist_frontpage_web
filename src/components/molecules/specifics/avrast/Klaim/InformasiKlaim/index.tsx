...
const InformasiKlaimComponent: React.FC<InformasiKlaimComponentProps> = ({ onTabChange, isSelectedDetail, onChangeBannerImg }) => {
  const [selected, setSelected] = useState('Informasi Klaim');
  ...
  return (
    <div className="w-full flex flex-col self-stretch items-center justify-center md:px-20 xs:px-4 bg-white py-20">
      ...
      {selected === 'Panduan & Pengajuan' && !isSelectedDetail && <PanduanPengajuanTab />}
    </div>
  );
};
