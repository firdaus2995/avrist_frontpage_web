...
const SliderComponent = ({ selected, slideItems, onClickItem }: Props) => {
  ...
  useEffect(() => {
    const activeIndex = slideItems.findIndex((item) =>
      typeof item === 'object' ? item.title === selected : item === selected
    );
  ...
  return (
    ...
    >
      {slideItems.map((tab) => {
        const activeTab = typeof tab === 'object' ? tab.title : tab;
        return (
          <Button
            key={activeTab}
            title={activeTab}
